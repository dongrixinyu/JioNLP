...
class F1(object):
    """ 计算 NER、语义信息抽取等序列标注任务的 F1 值。

    """
    def __init__(self, skip_labels=['E', 'I', 'M']):
        self.records = list()
        self.stats = dict()

        self.labels = dict()
        self.max_label_length = 10
        self.SKIP_LABEL = skip_labels
        self.ZERO_LABEL = 'O'
    ...
    def _update_results(self):
        ...
        self.confusion_matrix = np.zeros(shape=[num_label, num_label], dtype=np.int32)

        label2id = {label: idx for idx, label in enumerate(self.labels)}

        zero_idx = label2id[self.ZERO_LABEL]  # 在 NER 任务中才有
        ...
        for idx in range(num_records):
            ...
            if gold == pred:
                ...
                if not is_match:
                    self.confusion_matrix[zero_idx, pred_idx] += 1
                    self.confusion_matrix[gold_idx, zero_idx] += 1
                    continue
    ...
    def print_f_metric(self):
        ...
        for label in self.labels:
            if label == self.ZERO_LABEL or label[0] in self.SKIP_LABEL:
                continue
        ...
        return message, precision, recall, average_f1
