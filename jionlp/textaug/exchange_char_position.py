...
class ExchangeCharPosition(object):
    ...
    Args:
        ...
        exchange_ratio(float): 对每一个汉字的调整其位置概率，默认为 0.02
        seed(int): 控制随机交换位置每次不变，默认为 1
    ...
    def _prepare(self, augmentation_num=3, exchange_ratio=0.03,
                 seed=1, scale=1.0):
        ...
        self.jieba_obj = jieba.initialize()

    def _augment_one(self, text):
        char_list = list(text)
        for i in range(len(char_list)):
            if np.random.uniform(0, 1) < self.exchange_ratio:
                if not check_chinese_char(char_list[i]):
                    continue
                change_i = self._exchange_position(char_list, i)
                # print(i, change_i)
    ...
    def _exchange_position(self, char_list, orig_pos):
        # 找到可交换取值范围
        start_pos = 0
        end_pos = -1
    ...
    def __call__(self, text, augmentation_num=3, exchange_ratio=0.02,
                 seed=1, scale=1.0):
        if self.random is None or self.augmentation_num != augmentation_num \
            or self.exchange_ratio != exchange_ratio \
                or self.seed != seed or self.scale != scale:
            self._prepare(augmentation_num=augmentation_num,
        ...
        while len(augmentation_text_list) < self.augmentation_num:
            augmented_text = self._augment_one(text)
            count += 1
            if count > min(self.augmentation_num / self.exchange_ratio, len(text)):
                break
